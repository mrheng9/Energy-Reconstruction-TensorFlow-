...
kTwoProng = Cut(lambda tables:
                (tables['rec.vtx.mlvertex.fuzzyk']['npng'] == 2).groupby(level=KL).agg(np.any))
...
def kGammaCut(tables):
    df = tables['rec.vtx.mlvertex.fuzzyk.png.cvnpart']['photonid']
...
kPlaneGap = Cut(lambda tables:
                    (tables['rec.vtx.mlvertex.fuzzyk.png']['maxplanegap'] > 1).\
                    groupby(level=KL).agg(np.all))

kPlaneContig = Cut(lambda tables:
                        (tables['rec.vtx.mlvertex.fuzzyk.png']['maxplanecont'] > 4).\
                        groupby(level=KL).agg(np.all))
...
def kMass(tables):
    ...
    check = tables['rec.vtx.mlvertex.fuzzyk']['npng'] == 2

    df = tables['rec.vtx.mlvertex.fuzzyk.png'][check]
    x = df['dir.x']
    y = df['dir.y']
    z = df['dir.z']
