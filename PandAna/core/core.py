...
class spectrum():
    ...
    def __and__(self, b):
        assert self._POT == b._POT, "Spectra should have same POT"
        if self._weight is not None and b._weight is not None: 
            assert np.array_equal(self._weight, b._weight), "Spectra should have same weights"

        df = pd.concat([self._df, b._df], axis=1)
        pot = self._POT
        weight = self._weight
        return filledSpectrum(df, pot, weight=weight)
...
def save_tree(fname, spectra, groups, attrs=True):
    ...
    for spectrum, group in zip(spectra, groups):
        ...
        if type(df) == pd.Series:
            g.create_dataset('name', data=np.array([df.name],dtype='S'+str(len(df.name))))
        else:
            g.create_dataset('name', data=df.columns.values.astype('S'))
        if attrs:
        ...
        for name in index:
            ...
            if name == 'subevt' and group.startswith('particle_'):
                continue
...
def load_tree(fname, groups, attrs=True):
    ...
    for group in groups:
        ...
        cols = ['df0']
        if len(data.shape) > 1:
            cols = ['df'+str(i) for i in range(data.shape[1])]
        coldata = [data]
        if len(data.shape) > 1 and data.shape[1] != 1:
            coldata = [data[:,i] for i in range(data.shape[1])]
        spec = dict(zip(cols, coldata)) 
        index = []
        for key in f[group].keys():
            if key != 'pot' and key != 'weights' and key != 'df' and key != 'name':
                spec[key] = f[group+'/'+key][()]
                index.append(key)
        df = pd.DataFrame(spec)
        df.set_index(index, inplace=True)
        if 'name' in f[group].keys():
            name = f[group+'/name'][()]
            df.columns = name

        if attrs:
            pot = f[group+'/pot'][()]
            weights = pd.Series(f[group+'/weights'][()], df.index, name='weight')
            ret.append(filledSpectrum(df, pot, weight=weights))
        else:
            ret.append(df)
    
    f.close()
    return dict(zip(groups, ret))
...
class loader():
    ...
    def __setitem__(self, key, df):
        ...
        index = KL if (key.startswith('rec') or key in ['hits','edeps']) else KLN if (key.startswith('neutrino') or key in ['particles'])  else KLS
...
class interactive_loader():
    ...
    def __getitem__(self, key):
        if not key in self._tables:
            ...
            if key=="particles":
                df.set_index(KLN, inplace=True)
            self._tables[key] = df
        return self._tables[key]


"""
This function is passed a complete cut
and returns a function that loops through all 
of the cuts functions that return boolean series
without modification to tables
"""
def CutVar(kACut, name):    
    def fun(tables):
        df = kACut._cut[0](tables)
        for i in range(1, len(kACut._cut)):
            newdf = kACut._cut[i](tables)
            if newdf.empty and type(newdf) is pd.DataFrame: continue
            df &= newdf
        df.name = name
        return df
