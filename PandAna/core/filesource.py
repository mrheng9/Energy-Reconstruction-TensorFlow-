...
class samprojectsource():
  ...
  def setup(self):
    #  self.checkproxy()  # turning this off temporarily before I figure our what's going wrong with kx509 on the grid, seemingly only for pandana
    self.establishProcess()

  def checkproxy(self):
    try:
      #  subprocess.check_output(["klist", "-l"])
      check = subprocess.check_call(["klist", "-s"]) or subprocess.check_call(["klist", "-5", "-s"])
      if not check:
        self.hasTicket = True
    except subprocess.CalledProcessError:
      try: 
        subprocess.check_call(["setup_fnal_security", "-b"])
        self.hasTicket = True
      except subprocess.CalledProcessError:
        self.hasTicket = False
